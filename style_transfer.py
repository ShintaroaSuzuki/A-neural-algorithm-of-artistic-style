...
INI_IMAGE = 'content'  # 初期値にどちらの画像を与えるか['content', style]

OUTPUT_DIR = 'results'  # 生成画像ディレクトリ
if ITERATION == 5000:
    OUTPUT_IMG = os.path.splitext(os.path.basename(CONTENT_IMG))[0] + '_by_' + os.path.splitext(os.path.basename(STYLE_IMG))[0] + '_' + str(STYLE_STRENGTH) + '_' + INI_IMAGE + '.png'  # 生成画像ファイル
else:
    OUTPUT_IMG = os.path.splitext(os.path.basename(CONTENT_IMG))[0] + '_by_' + os.path.splitext(os.path.basename(STYLE_IMG))[0] + '_' + str(STYLE_STRENGTH) + '_' + INI_IMAGE + '_2.png'  # 生成画像ファイル
...
sess.run(net['input'].assign( INI_NOISE_RATIO* noise_img + (1.-INI_NOISE_RATIO) * IMAGE[INI_IMAGE]))
...
# save as png
plt.savefig("kyoto1_by_rembrandt-loss.png")
"""
if ITERATION == 5000:
    plt.savefig(os.path.splitext(os.path.basename(CONTENT_IMG))[0] + '_by_' + os.path.splitext(os.path.basename(STYLE_IMG))[0] + '_' + str(STYLE_STRENGTH) + '-loss.png')
else:
    plt.savefig(os.path.splitext(os.path.basename(CONTENT_IMG))[0] + '_by_' + os.path.splitext(os.path.basename(STYLE_IMG))[0] + '_' + str(STYLE_STRENGTH) + '_2-loss.png')
"""
